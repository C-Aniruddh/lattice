...
import { GRASS, RIVER, type Valley } from './valley.js';
...
export function smoke(pen: Pen, gx: number, gy: number, zPx: number, seed: number, strength: number): void {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    s.softEllipse(
      ...
      r,
      r * 0.86,
      withAlpha(mix(pen.palette.get('metal'), pen.palette.get('sky'), 0.5), (1 - phase) * 0.3 * strength),
      withAlpha(pen.palette.get('sky'), 0),
    );
  }
}
...
function birds(pen: Pen, v: Valley, daylight: number): void {
  ...
  for (let i = 0; i < BIRDS; i++) {
    ...
    const spread = (i % 5) - 2;
    const gx = phase * 54 - 7 + spread * 1.5;
    const gy = 3 + lane * 34 - spread * 2.0 + Math.sin(pen.t * 0.7 + i) * 0.4; /* @tier-b pixels only */
...
function fireflies(pen: Pen, v: Valley, daylight: number): void {
  ...
  for (let i = 0; i < FLIES; i++) {
    const gx = 4 + toUnit(hash2(v.seed ^ 0xf1, i, 1)) * 32 + noise2(v.seed, i * 2.3, pen.t * 0.22) * 2.6;
    const gy = 4 + toUnit(hash2(v.seed ^ 0xf1, i, 2)) * 32 + noise2(v.seed, i * 5.7, pen.t * 0.19) * 2.6;
    ...
    s.softEllipse(pt.x, pt.y, r * 5, r * 5, withAlpha(warm, a * 0.22), withAlpha(warm, 0));
...
function motes(pen: Pen, v: Valley, daylight: number): void {
  ...
  for (let i = 0; i < MOTES; i++) {
    const gx = 3 + toUnit(hash2(v.seed ^ 0x7c, i, 1)) * 34 + noise2(v.seed, i * 1.9, pen.t * 0.14) * 3.4;
    const gy = 3 + toUnit(hash2(v.seed ^ 0x7c, i, 2)) * 34 + noise2(v.seed, i * 8.1, pen.t * 0.12) * 3.4;
...
function deer(pen: Pen, v: Valley, daylight: number): void {
  for (let i = 0; i < DEER; i++) {
    const cx = 6 + toUnit(hash2(v.seed ^ 0xd3, i, 1)) * 28;
    const cy = 6 + toUnit(hash2(v.seed ^ 0xd3, i, 2)) * 28;
    ...
    pen.surface.softEllipse(pt.x, pt.y + 2 * k, 7 * k, 3.4 * k, withAlpha(pen.palette.get('ink'), 0.3), withAlpha(pen.palette.get('ink'), 0));
