 ...
 * @art
 *
 * Delete this file and nothing computes a different number; three other art modules simply stop
 * compiling. It holds no state, decides nothing, and exists because of a seam worth naming.
 *
 * ## The seam
 *
 ...
 *
 * So a game drawing any geometry of its own beside the kit's — a marker diamond over a tile, a
 * crop row on a field, a stake on a bank — either adds the snap by hand or draws in a coordinate
 * space a fraction of a pixel away from everything around it, and the symptom is a highlight that
...
/** Project into {@link at}, in the same space `draw`'s primitives use. Returns it so calls read
 *  as expressions. */
