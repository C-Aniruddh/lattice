...
<a href="https://lattice.plausible.ventures"><img src="https://lattice.plausible.ventures/og.png" alt="A valley at dusk rendered by Lattice: an isometric hillside with a lit shrine and a road of lamps" width="860"></a>
...
|---|---|---|---|
| **Before the Bell** | Grok | *place stalls and open gates to pull the crowd to your bakery before the market closes* | [play](https://lattice.plausible.ventures/g/before-the-bell/) |
| **Chime Path** | Claude | *hang chimes along a mountain path and tune each one, so the wind plays them in order as walkers pass* | [play](https://lattice.plausible.ventures/g/chime-path/) |
| **Evenfall Orchard** | Codex | *plant an orchard and each evening choose to harvest or let it grow, and it keeps growing while the tab is closed* | [play](https://lattice.plausible.ventures/g/evenfall-orchard/) |
...
otherwise be a claim. **[See them running →](https://lattice.plausible.ventures)**
...
inside it. It is [the file the landing page runs beside its own source](https://lattice.plausible.ventures/example/).
...
package's exports and invariants. [`/api.json`](https://lattice.plausible.ventures/api.json) and
[`/llms.txt`](https://lattice.plausible.ventures/llms.txt) are the versions an agent reads.
...
| the API reference | **generated, not hand-kept** | [`/reference/`](https://lattice.plausible.ventures/reference/) is read out of the packages' own `.d.ts` files at build time |
...
machine-readable at [`/api.json`](https://lattice.plausible.ventures/api.json), which is a thing you
