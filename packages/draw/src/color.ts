/**
 * Colour: one packed integer per colour, and the three-tone face derivation the look rests on.
 *
 ...
 * derived token and you have written an engine-specific artefact into a document that will be
 ...
 * because a colour is ever hashed (it must not be), but because it costs nothing here and it
 ...
 * `Map` with no hashing, and hand a WebGL backend its vertex colour with two shifts.
 *
 ...
 * to the same colour produced anywhere else, which is a cache key that never hits.
 */
 ...
 *  how a second colour model starts. */
 ...
 * that may legitimately overshoot — `shade(c, 1.4)` on an already-bright colour — and throwing
 ...
 * flat grey lerp. Neutralise this to grey and every screenshot still renders and every
 ...
 * about 0.25 the faces read as a grey multiply; above about 0.6 every building in the world
 * has the same two colours in it and the palette stops mattering.
 */
 ...
 * top face is drawn at `FACE_TOP`, and a top face that is not bit-identical to the colour the
...
/** How far toward black an outline sits. Not zero: a pure black outline against a coloured
 ...
 *
 * Derived from the solid's own colour rather than fixed, so a brand recolour moves the
...
/**
 * How many packed colours the CSS memo holds before it is dropped wholesale.
 *
 * A palette plus its derived faces is a few hundred entries; anything past this is a caller
 * generating colours per frame, and an unbounded memo would then be a leak that looks like a
...
/**
 * Packed colour → a CSS `rgb()`/`rgba()` string, memoised. **Backends only; not for game code.**
 *
 * This is the *only* place in the kit where a colour becomes a string, and it exists because
 ...
 * never in a save** — it is derived, and derived colour is presentation-tier.
 */
...
/**
 * Parse `#rgb`, `#rrggbb` or `#rrggbbaa` into a packed colour.
 *
 ...
 *
 * Hue is how a *player* picks a brand colour — a wheel, one number — and how a theme derives a
 ...
 *
 * `h` wraps, so 380 and 20 are the same colour and a hue driven by an accumulating slider
...
export function hsl(h: number, s: number, l: number, a = 1): Rgba {
  ...
  if (sat === 0) {
    ...
    return rgba(grey, grey, grey, a * 255);
  }
 ...
 * theme from one hue and must not grow a second colour model to do it. **Colour lives in
